...
const char* ToString(CBlockDetectionPipeline::SImageBuffer::EState e_state) {
   switch(e_state) {
      case CBlockDetectionPipeline::SImageBuffer::EState::IDLE:
         return "IDLE";
         break;
      case CBlockDetectionPipeline::SImageBuffer::EState::READY:
         return "READY";
         break;
      default:
         return "UNDEF";
...
const char* ToString(CBlockDetectionPipeline::SBlockListBuffer::EState e_state) {
   switch(e_state) {
      case CBlockDetectionPipeline::SBlockListBuffer::EState::IDLE:
         return "IDLE";
         break;
      case CBlockDetectionPipeline::SBlockListBuffer::EState::READY:
         return "READY";
         break;
      default:
         return "UNDEF";
...
CBlockDetectionPipeline::CBlockDetectionPipeline(CISSCaptureDevice* pc_iss_capture_device,
   ...
   /* Debug */
   m_psImageBuffers[0].Id = "ImgA";
   m_psImageBuffers[1].Id = "ImgB";
   m_psBlockListBuffers[0].Id = "DetA";
   m_psBlockListBuffers[1].Id = "DetB";
      ...
      while(m_bPipelineEnable) {
         for(SImageBuffer& s_image_buffer : m_psImageBuffers) {
            if(s_image_buffer.Mutex.try_lock()) {
               //DebugMutex.lock();
               //Debug << "[Debug] Capture locked buffer #" << s_image_buffer.Id << ":" << ToString(s_image_buffer.State) << std::endl;
               //DebugMutex.unlock();
               if(s_image_buffer.State == SImageBuffer::EState::IDLE) {
                  if(m_pcIssCaptureDevice->GetFrame(s_image_buffer.Y,
                     ...
                     DebugMutex.lock();
                     Debug << "[Debug] Writing frame #" << s_image_buffer.Index << " (" << s_image_buffer.Id << ")" << std::endl;
                     DebugMutex.unlock();
                  }
                  else {
                     DebugMutex.lock();
                     Debug << "[Debug] Failure!!" << std::endl;
                     DebugMutex.unlock(); 
                  }
               }
               s_image_buffer.Mutex.unlock();
               //DebugMutex.lock();
               //Debug << "[Debug] Capture unlocked buffer #" << s_image_buffer.Id << ":" << ToString(s_image_buffer.State) << std::endl;
               //DebugMutex.unlock();
            }
         }
      }
   ...
   m_fnDetectLoop = [this] {
      ...
      while(m_bPipelineEnable) {
         for(SImageBuffer& s_image_buffer : m_psImageBuffers) {
            if(s_image_buffer.Mutex.try_lock()) {
               //DebugMutex.lock();
               //Debug << "[Debug] Detect locked buffer #" << s_image_buffer.Id << ":" << ToString(s_image_buffer.State) << std::endl;
               //DebugMutex.unlock();
               if(s_image_buffer.State == SImageBuffer::EState::READY) {
                  for(SBlockListBuffer& s_block_list_buffer : m_psBlockListBuffers) {
                     if(s_block_list_buffer.Mutex.try_lock()) {
                        //DebugMutex.lock();
                        //Debug << "[Debug] Detect locked buffer #" << s_block_list_buffer.Id << ":" << ToString(s_block_list_buffer.State) << std::endl;
                        //DebugMutex.unlock();
                        if(s_block_list_buffer.State == SBlockListBuffer::EState::IDLE) {
                           ...
                           DebugMutex.lock();
                           Debug << "[Debug] Detecting from frame #" 
                                 << s_image_buffer.Index
                                 << " (" 
                                 << s_image_buffer.Id
                                 << ") to list #" 
                                 << s_block_list_buffer.Index
                                 << " ("
                                 << s_block_list_buffer.Id
                                 << ")"
                                 << std::endl;
                           DebugMutex.unlock();
                        }
                        ...
                        //DebugMutex.lock();
                        //Debug << "[Debug] Detect unlocked buffer #" << s_block_list_buffer.Id << ":" << ToString(s_block_list_buffer.State) << std::endl;
                        //DebugMutex.unlock();
                        break;
                     }
                  }
               ...
               s_image_buffer.Mutex.unlock();
               //DebugMutex.lock();
               //Debug << "[Debug] Detect unlocked buffer #" << s_image_buffer.Id << ":" << ToString(s_image_buffer.State) << std::endl;
               //DebugMutex.unlock();
            }
         }
      }
...
void CBlockDetectionPipeline::WriteDebugToScreen() {
   DebugMutex.lock();
   std::cerr << Debug.str();
   Debug.str("");
   DebugMutex.unlock();
      ...
      for(SBlockListBuffer& s_block_list_buffer : m_psBlockListBuffers) {
         if(s_block_list_buffer.Mutex.try_lock()) {
            if(s_block_list_buffer.State == SBlockListBuffer::EState::READY) {
               ...
               DebugMutex.lock();
               Debug << "[Debug] Reading blocks from list #"
                     << s_block_list_buffer.Index 
                     << " ("
                     << s_block_list_buffer.Id 
                     << ")"
